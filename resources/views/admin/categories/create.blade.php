<x-admin-layout>
    <div class="max-w-2xl mx-auto">
        <div class="mb-10">
            ...
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Tambah Bidang Baru</h1>
            <p class="text-slate-500 text-sm mt-1">Buat kategori industri untuk klasifikasi perusahaan.</p>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 md:p-12">
            <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-8">
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" class="block w-full bg-slate-50 border-transparent focus:bg-white transition" :value="old('name')" required placeholder="Contoh: Fintech, Media, BUMN" />
