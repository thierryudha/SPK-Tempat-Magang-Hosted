<x-admin-layout>
    <div class="max-w-3xl mx-auto">
        <div class="mb-10">
            ...
            <h1 class="text-3xl font-black text-slate-900">Edit Kriteria</h1>
            <p class="text-slate-500 text-sm mt-2">Perbarui definisi kriteria <span class="text-blue-600 font-black">{{ $criteria->name }}</span>.</p>
        </div>

        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 md:p-12">
            <form action="{{ route('admin.criterias.update', $criteria) }}" method="POST" class="space-y-10">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <x-text-input name="scales[{{ $i }}]" type="text" class="block w-full bg-slate-50 border-transparent focus:bg-white transition text-xs font-bold" :value="old('scales.'.$i, $scale ? $scale->description : '')" required />
